 ...
 *
 * This file depends on the q.js Promise library.
 *
 ...
 *
 * The Scalyr Backend is suppose to simulate AJAX calls to a real backend.  As such, all the public
 * methods for the Scalyr Backend return a promise, which can be used to perform work once the
 * simulated asynchronous AJAX calls are completed.
 *
 * The two main methods you will be using are:
 *
 ...
 *         the specified list of users have been added to the backend.  Each user in the list will
...
(function(window, Q) {
  ...
  function ScalyrBackend() {
    ...
    this._userList = [
      ...
      this._createUserRecord('tracy@scalyr.com', AccessLevel.FULL, UserState.INVITED),
      this._createUserRecord('chris@scalyr.com', AccessLevel.READ_ONLY, UserState.ACTIVE),
  ...
  ScalyrBackend.prototype.revokeAccess = function revokeAccess(user) {
    var targetUser = this._findUser(user);
    if (!targetUser) {
      return this._returnError('userNotExists', 'The specified user "' + user + '" does not exists.');
    }
  ...
  // Export the backend to the global window object.
  window['scalyrBackend'] = new ScalyrBackend();
